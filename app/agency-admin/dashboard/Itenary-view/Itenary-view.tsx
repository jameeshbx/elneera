...
const parseCSV = (csvText: string): CsvItineraryData | null => {
  try {
    const lines = csvText.split('\n').filter(line => line.trim() !== '');
    if (lines.length < 2) return null;

    // Parse metadata from the first two lines
    const headers = lines[0].split(',').map(h => h.trim());
    const metaValues = lines[1].split(',').map(v => v.trim());
    
    const metadata: Record<string, string> = {};
    headers.forEach((header, index) => {
      metadata[header] = metaValues[index] || '';
    });

    // Parse activities
    const activitiesByDay: Record<number, CsvActivity[]> = {};

    // Start from line 3 (index 2) to skip the header and metadata rows
    for (let i = 2; i < lines.length; i++) {
      const [day, time, title] = lines[i].split(',').map(s => s.trim());
    ...
    // Convert to daily itinerary format
    const dailyItinerary = Object.entries(activitiesByDay).map(([day, activities]) => ({
      day: parseInt(day),
      date: new Date(metadata.startDate || new Date())
        .setDate(new Date(metadata.startDate || new Date()).getDate() + parseInt(day) - 1)
        .toLocaleString(),
      title: `Day ${day}`,
      description: `Activities for day ${day}`,
    ...
    return {
      ...
      name: metadata.name || 'Untitled Itinerary',
      days: parseInt(metadata.days) || dailyItinerary.length,
      title: metadata.title || 'Your Travel Itinerary',
      description: metadata.description || '',
      nights: parseInt(metadata.nights) || 0,
      startDate: metadata.startDate || new Date().toISOString().split('T')[0],
      costINR: parseFloat(metadata.costINR) || 0,
      costUSD: parseFloat(metadata.costUSD) || 0,
      guests: parseInt(metadata.guests) || 2,
      adults: parseInt(metadata.adults) || 2,
      kids: parseInt(metadata.kids) || 0,
      dailyItinerary,
      location: metadata.location || '',
      richContent: ''
    };
...
export function ItineraryViewContent() {
  ...
  // Function to load saved CSV data from localStorage
  const loadSavedCsvData = useCallback((csvData: CsvItineraryData): CsvItineraryData => {
    if (typeof window !== 'undefined') {
      const storageKey = `csv_itinerary_${csvData.quoteId}`;
      const savedData = localStorage.getItem(storageKey);
      
      if (savedData) {
        try {
          const parsedSavedData = JSON.parse(savedData);
          console.log('Loaded saved CSV data from localStorage:', parsedSavedData.quoteId);
  ...
  const fetchAndParseCsv = useCallback(async (filename: string): Promise<CsvItineraryData | null> => {
    try {
      ...
      console.log('Fetched CSV content:', csvText.substring(0, 200) + '...');
      ...
      // Load any saved modifications from localStorage
      const savedData = loadSavedCsvData(parsedData);
      return savedData;
    } catch (error) {
  ...
  const findAndLoadMatchingCsv = useCallback(async (location: string): Promise<boolean> => {
    try {
      ...
      if (matchingFile) {
        ...
        if (csvData) {
          console.log('Successfully parsed CSV data with saved modifications');
  ...
  useEffect(() => {
    const fetchItineraryData = async () => {
      if (!itineraryData?.location) return;
      ...
      try {
        ...
        const locationKey = itineraryData.location.toLowerCase();
        const csvFile = locationMap[locationKey];

        if (!csvFile) {
          console.error('No CSV file found for location:', itineraryData.location);
          return;
        }

        const response = await fetch(`/Itinerary/${csvFile}.csv`);
        ...
        if (csvData) {
          setItineraryData(prev => ({
            ...prev!,
            dailyItinerary: csvData.dailyItinerary as ExtendedDayItinerary[],
            title: csvData.title,
            description: csvData.description,
            days: csvData.days,
            nights: csvData.nights,
            costINR: csvData.costINR,
            costUSD: csvData.costUSD,
            guests: csvData.guests,
            adults: csvData.adults,
            kids: csvData.kids,
            locationMatched: csvData.location
          }));
        }
      } catch (error) {
  ...
  const handleSaveRichText = useCallback(async () => {
    ...
    try {
      ...
      // For CSV data, we'll update the local state and localStorage for persistence
      if (useCSVData && csvItineraryData) {
        let updatedCsvData;
        
        if (editingDay !== null) {
          // Update specific day's rich content in CSV data
          updatedCsvData = {
            ...csvItineraryData,
            dailyItinerary: csvItineraryData.dailyItinerary.map((day) => ({
              ...day,
              richContent: day.day === editingDay ? richTextContent : day.richContent,
            }))
          };
        } else {
          // Update overall rich content in CSV data
          updatedCsvData = {
            ...csvItineraryData,
        ...
        setCsvItineraryData(updatedCsvData);
        
        // Save to localStorage for persistence across page refreshes
        if (typeof window !== 'undefined') {
          const storageKey = `csv_itinerary_${updatedCsvData.quoteId}`;
          localStorage.setItem(storageKey, JSON.stringify(updatedCsvData));
        }
        
        alert("Itinerary data saved successfully!");
      ...
      // For database data, save to the API
      if (itineraryData && 'id' in itineraryData) {
        const updateData: Record<string, unknown> = {
          updatedAt: new Date().toISOString(),
        };
  
        if (editingDay !== null) {
          // Update specific day's rich content
          updateData.dailyItinerary = itineraryData.dailyItinerary?.map((day) => ({
            ...day,
            richContent: day.day === editingDay ? richTextContent : day.richContent,
          })) || [];
        } else {
          // Update overall rich content
          updateData.richContent = richTextContent;
        }
  
        const response = await fetch(`/api/itineraries/${itineraryData.id}`, {
          method: "PUT",
          headers: {
        ...
        setItineraryData(prev => {
          ...
          if (editingDay !== null) {
            return {
              ...prev,
              dailyItinerary: prev.dailyItinerary?.map((day) => ({
                ...day,
                richContent: day.day === editingDay ? richTextContent : day.richContent,
              })) || []
            };
  ...
  const handleExportPDF = async () => {
    ...
    try {
        ...
        <html>
          <head>
            ...
            <style>
              ...
              .rich-content h1, .rich-content h2, .rich-content h3, .rich-content h4 { margin-top: 20px; margin-bottom: 10px; }
              .rich-content p { margin-bottom: 15px; }
              .rich-content ul, .rich-content ol { margin-bottom: 15px; padding-left: 20px; }
              @media print {
                body { padding: 0; }
      ...
      // Include overall rich content if available
      const currentData = useCSVData ? csvItineraryData : itineraryData;
      if (currentData?.richContent) {
        htmlContent += `<div class="rich-content">${currentData.richContent}</div>`
      }
  ...
  const handleEditDailyItinerary = (dayNumber?: number) => {
    ...
    if (dayNumber) {
      ...
      if (dayData) {
        // Check if there's already rich content saved for this day
        if (dayData.richContent) {
          setRichTextContent(dayData.richContent)
        } else {
          // Generate default rich content
      ...
      if (currentData && currentData.dailyItinerary) {
        // Check if there's already rich content saved
        const existingRichContent = useCSVData ? csvItineraryData?.richContent : itineraryData?.richContent;
        
        if (existingRichContent) {
          setRichTextContent(existingRichContent)
        } else {
          // Generate default rich content
              ...
              {currentDisplayData && (
                <div className="flex items-center gap-2">
                  <span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full">
                    {useCSVData ? 'CSV' : 'DB'} Data: {currentDisplayData.name}
                  </span>
          ...
          <div className="lg:col-span-3" id="itinerary-content">
            ...
            <div className="bg-white rounded-lg shadow overflow-y-auto">
              <div className="p-6">
                ...
                {/* Display saved rich content if available */}
                {(useCSVData ? csvItineraryData?.richContent : itineraryData?.richContent) && (
                  <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                    <div 
                      className="prose max-w-none"
                      dangerouslySetInnerHTML={{ 
                        __html: (useCSVData ? csvItineraryData?.richContent : itineraryData?.richContent) || ''
                      }}
                    />
                  </div>
                )}
                ...
                <div className="space-y-4">
                  {(currentDisplayData?.dailyItinerary || []).map((day) => (
                    <div key={day.day} className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                      {/* Day Header */}
                      <div 
                        className="p-4 bg-gray-50 cursor-pointer flex justify-between items-center hover:bg-gray-100 transition-colors"
                      ...
                      {expandedDays[day.day] && (
                        <div className="bg-white">
                          {/* Display saved rich content for this day */}
                          {day.richContent && (
                            <div className="p-4 bg-blue-50 border-b">
                              <div 
                                className="prose max-w-none text-sm"
                                dangerouslySetInnerHTML={{ __html: day.richContent }}
                              />
                            </div>
                          )}
                              ...
                              {day.activities.map((activity, activityIndex) => (
                                ...
                                >
                                  ...
                                  <div className="flex-shrink-0 mt-1">
                                    {activity.type === 'meal' || activity.title.toLowerCase().includes('breakfast') || activity.title.toLowerCase().includes('lunch') || activity.title.toLowerCase().includes('dinner') ? (
                                      <div className="text-lg">🍽️</div>
                                    ) : activity.type === 'transfer' || activity.title.toLowerCase().includes('pickup') || activity.title.toLowerCase().includes('transfer') ? (
                                      <div className="text-lg">🚐</div>
                                    ) : activity.type === 'adventure' || activity.title.toLowerCase().includes('adventure') || activity.title.toLowerCase().includes('climbing') || activity.title.toLowerCase().includes('trekking') ? (
                                      <div className="text-lg">⛰️</div>
                                    ) : activity.title.toLowerCase().includes('tea') ? (
                                      <div className="text-lg">🫖</div>
                                    ) : activity.title.toLowerCase().includes('horse') || activity.title.toLowerCase().includes('riding') ? (
                                      <div className="text-lg">🐎</div>
                                    ) : activity.title.toLowerCase().includes('cultural') ? (
                                      <div className="text-lg">🎭</div>
                                    ) : (
                                      <div className="text-lg">📍</div>
                                    )}
                                    ...
                                    <p className="text-sm text-gray-600 leading-relaxed">
                                      {activity.description || (() => {
                                        ...
                                        if (title.includes('breakfast')) {
                                          return 'Start your day with traditional local breakfast featuring regional specialties and fresh ingredients';
                                        } else if (title.includes('lunch')) {
                                          return 'Enjoy authentic local cuisine with stunning views and traditional flavors of the region';
                                        } else if (title.includes('dinner')) {
                                          return 'Delicious dinner featuring local specialties and traditional cooking methods of the area';
                                        } else if (title.includes('pickup') || title.includes('transfer')) {
                                          return 'Comfortable hotel pickup and transfer service for your exciting adventure in the region';
                                        } else if (title.includes('adventure') || title.includes('trekking') || title.includes('climbing')) {
                                          return 'Thrilling outdoor adventure activities including exciting experiences in beautiful natural settings';
                                        } else if (title.includes('tea')) {
                                          return 'Experience authentic local tea culture with traditional preparation methods and local varieties';
                                        } else if (title.includes('horse') || title.includes('riding')) {
                                          return 'Scenic horse riding experience through beautiful landscapes and picturesque valleys';
                                        } else if (title.includes('cultural')) {
                                          return 'Traditional cultural performances and local handicraft demonstrations showcasing regional heritage';
                                        } else if (title.includes('shopping')) {
                                          return 'Explore local markets and shops for authentic souvenirs and traditional handicrafts';
                                        } else if (title.includes('sightseeing')) {
                                          return 'Visit famous landmarks and attractions with guided tours and historical insights';
                                        } else {
                                          return 'Exciting activity experience designed to showcase the best of local culture and attractions';
                                        }
                                      })()}
                                    </p>
                                  </div>
                                  ...
                                  <div className="flex-shrink-0">
                                    <button 
                                      ...
                                      onClick={() => handleEditDailyItinerary(day.day)}
                                    >
                                      Details
                                    </button>
                                  </div>
                  ...
                  {/* Empty State */}
                  {(!currentDisplayData?.dailyItinerary && !currentFallbackData?.dailyItinerary) && (
                    <div className="text-center py-12 bg-gray-50 rounded-lg">
                      <Calendar className="w-12 h-12 mx-auto mb-4 text-gray-400" />
                      <h3 className="text-lg font-semibold text-gray-700 mb-2">No Itinerary Available</h3>
                      <p className="text-gray-500 mb-4">Your travel itinerary will appear here once created.</p>
                      <Button 
                        onClick={() => handleEditDailyItinerary()}
